...
from basic_memory.utils import coerce_dict, validate_project_path
...
## How to retry
1. Use `list_memory_projects()` to confirm the project and project ID.
...
async def edit_note(
    ...
    # Resolve a routable identifier prefix before selecting a client.
    # Trigger: no explicit project/project_id was provided and the identifier is a
    #   memory:// URL.
    # Why: memory:// URLs are explicit routes, but a plain path is ambiguous for a
    #   mutating tool — "research/note" is far more often a directory than a project.
    # Outcome: memory:// can route; plain identifiers stay in the active project.
