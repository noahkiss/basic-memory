...
| `bm project add <name> --home-here` | Create a project whose notes live in `./.bm` instead of the store, for a directory something else already versions — a Claude Code skill yadm carries between machines. The vocabulary travels beside the records. Implies `--here` over the whole tree, takes no path argument, and never resolves `./.bm`, which under yadm's link mode is a per-machine symlink. |
| `bm project adopt [<name>]` | Register a project whose notes another VCS delivered to this machine — yadm has carried a skill's `.bm/` here, and this machine's registry knows nothing about it. Resolves by name, because names travel between machines and ids do not. Writes the marker, indexes what arrived, and changes nothing when run again. Refuses when `./.bm` is not here yet. |
